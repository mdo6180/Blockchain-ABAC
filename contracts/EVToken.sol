...
contract EVToken {
    ...
    // FUNCTIONS
    constructor(uint256 total, uint256 expiration_time)
    {
        total_supply = total;
        balances[msg.sender].balance = total_supply;
        balances[msg.sender].expiration_time = expiration_time;
        admin = msg.sender;
    }
    ...
    // params: 
    //      token_owner (address): wallet address of token owner
    ...
    // params:
    //      receiver (address): wallet address of receiver
    //      num_tokens (uint256): number of tokens to transfer to receiver
    // returns:
    ...
    // params:
    //      delegate (address): wallet address of third party
    //      num_tokens (uint256): number of tokens to allow to delegate to send
    // returns:
    //      True if transfer was successful, false otherwise
    function approve(
        address delegate,
